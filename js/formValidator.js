...
const isValidTelefone = (tel) => {
  // Regex para formato (00) 00000-0000 ou (00) 0000-0000 (aceita 4 ou 5 dígitos no meio)
...
const validateForm = (event) => {
  ...
  // Obtenção dos Inputs (certifique-se de que os IDs #cpf, #telefone, #cep existem no template!)
  const nomeInput = form.querySelector('#nome');
  const emailInput = form.querySelector('#email');
  const cpfInput = form.querySelector('#cpf');
  const telefoneInput = form.querySelector('#telefone');
  const cepInput = form.querySelector('#cep');
  // Se você tiver senhas novamente, adicione as referências aqui:
  // const senhaInput = form.querySelector('#senha'); 

  // 1. Nome (Exemplo: Mínimo 5 caracteres)
  ...
  if (isValid) {
    alert('✅ Formulário validado! Seu código JS está funcionando.');
    form.reset(); 
  }
};
